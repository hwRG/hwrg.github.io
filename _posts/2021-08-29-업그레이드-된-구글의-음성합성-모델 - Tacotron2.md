...
tags: [Training]
math: true
image: /_posts_img/tacotron2/tacotron2_diagram.png
---
...
![tacotron2_diagram](/_posts_img/tacotron2/tacotron2_diagram.png)
...
![encoder_entire](/_posts_img/tacotron2/encoder.png)

구성은 순서대로 **char embedding → 3 x 1D Convolution Layer → Bidirection LSTM**이다.<br/><br/>
처음 input으로 one-hot vector 정수 들어오면 embedding matrix로 512차원 vector로 변환 후,<br/>
vector는 1D conv-layer를 지나 bi-LSTM에서 encoded feature로 변환된다.<br/><br/>

 ![encoder_zoneout](/_posts_img/tacotron2/encoder_zoneout.png)(zoneout의 구조)
...
![attention](/_posts_img/tacotron2/attention.png)     
...
- 기존 attention과의 차이점은 score 계산 전에 이전 state가 포함된다.
  ![attention_LSA](_posts_img\tacotron2\attention_LSA.png)
...
mel-spectrogram을 이용하여 **다음 시점의 mel-spectrogram을 생성**한다.<br><br>![decoder](/_posts_img/tacotron2/decoder.png)

**모델 구성**
 Pre-Net → Decoder LSTM
→ Projection Layer → Post-Net
...
자세한 내용은 아래 주소에 정리해 두었으니 확인하길 바란다.<br>

https://hwrg.github.io/posts/%EB%94%A5%EB%9F%AC%EB%8B%9D-%EA%B8%B0%EB%B0%98%EC%9D%98-%EC%9D%8C%EC%84%B1%ED%95%A9%EC%84%B1-Vocoder-Wavenet/
